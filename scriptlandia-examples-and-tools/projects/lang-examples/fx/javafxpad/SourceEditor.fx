...
operation SourceEditor.createView():<<javax.swing.JComponent>> {
    ...
    jtextarea.getDocument().addUndoableEditListener(new UndoableEditListener() {
            operation undoableEditHappened(e:UndoableEditEvent) {
                ...
                var c = ++count;
                for (i in [false, true]) (dur 1000 linear while count == c) { 
                    if (i and c == count) {
    ...
    var z = <<javax.swing.KeyStroke>>.getKeyStroke(Z:KeyStroke.id, CTRL:KeyModifier.id);
    var y = <<javax.swing.KeyStroke>>.getKeyStroke(Y:KeyStroke.id, CTRL:KeyModifier.id);
