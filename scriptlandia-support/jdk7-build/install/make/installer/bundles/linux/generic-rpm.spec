...
%define jre_name                JRE_PKG_NAME
%define jdk_name                JDK_PKG_NAME
...
%define jre_docs                CHANGES|COPYRIGHT|LICENSE|README|Welcome.html
%define jdk_docs                COPYRIGHT|LICENSE|README.html|README_ja.html|README_zh_CN.html
...
EOF
    ...
    while [ -n "${path}" ]; do
        if [ "${path}" != "/" ]; then
            additional_dirs="%dir ${path}${additional_dirs:+\n${additional_dirs}}"
            path=`dirname ${path}`
        else
            path=
        fi
    ...
    #
    echo -e "${additional_dirs}" >> %{package_filelist}
